...
let privateConfig = {};
try {
  privateConfig = require('./server.private');
} catch {
  privateConfig = {};
}

function pickPrivate(name, fallback = '') {
  return privateConfig[name] || process.env[name] || fallback;
}
...
const SESSION_DAYS = Number(pickPrivate('SESSION_DAYS', 7));
...
const setupPassword = pickPrivate('SETUP_PASSWORD');
const setupPasswordHash = pickPrivate('SETUP_PASSWORD_HASH');
const setupPasswordSalt = pickPrivate('SETUP_PASSWORD_SALT');
const jwtSecret = pickPrivate('JWT_SECRET');
const appOrigin = pickPrivate('APP_ORIGIN');
const dropboxEmail = pickPrivate('DROPBOX_EMAIL');
const dropboxPass = pickPrivate('DROPBOX_PASS');
const adminPass = pickPrivate('ADMIN_PASS');

if (!jwtSecret) {
  console.error('Missing JWT_SECRET in server.private.js or environment.');
  process.exit(1);
}

if (!setupPassword && !(setupPasswordHash && setupPasswordSalt)) {
  console.error('Missing password config. Use SETUP_PASSWORD or SETUP_PASSWORD_HASH + SETUP_PASSWORD_SALT in server.private.js or environment.');
  process.exit(1);
}
